...
└─────────────────┘    └─────────────────┘    └─────────────────┘
    ...
    def parse_prd_content(self, prd_content: str) -> Dict[str, Any]:
        ...
        goals = []
        goal_patterns = [
            r'(?:goals?|objectives?|purpose)[:\s]*\n([\s\S]*?)(?=\n\d+\.|\n[A-Z][^\n]*:|$)',
            r'(?:the goal of this project is to|our objective is to)\s+([^.\n]+)'
        ]
        for pattern in goal_patterns:
            matches = re.findall(pattern, cleaned_content, re.IGNORECASE | re.MULTILINE)
            for match in matches:
                items = re.findall(r'(?:^|\n)\s*(?:[\-•*]|\d+\.?)?\s*([^\n]+)', match, re.MULTILINE) if '\n' in match else [match]
                for item in items:
                    clean_item = item.strip()
                    if 5 < len(clean_item) < 150:
                        goals.append(clean_item)

        # Extract audience with enhanced patterns
        audience = []
        audience_patterns = [
            r'(?:target audience|users?|persona[:\s]*)[:\s]*\n([\s\S]*?)(?=\n\d+\.|\n[A-Z][^\n]*:|$)',
        ...
        for pattern in audience_patterns:
            ...
            for match in matches:
                ...
                for item in items:
                    clean_item = item.strip()
                    if 5 < len(clean_item) < 150:
                        audience.append(clean_item)

        # Extract project scope
        scope = []
        scope_patterns = [
            r'(?:scope|boundaries|inclusive of)[:\s]*\n([\s\S]*?)(?=\n\d+\.|\n[A-Z][^\n]*:|$)'
        ]
        ...
        result = {
            ...
            'database_tables': database_tables[:10],
...
+-----------+     +-------------------+     +-------------------+     +-------------+
    ...
    def _create_business_alignment(self) -> List:
        ...
        goals = self._prd_analysis.get('goals', [])
        if goals:
            story.append(Paragraph("Project Goals & Objectives:", self.styles['CustomHeading2']))
            for goal in goals:
                story.append(Paragraph(f"• {self._sanitize_text(goal)}", self.styles['CustomBullet']))
            story.append(Spacer(1, 0.1*inch))
        
        # Target Audience
        audience = self._prd_analysis.get('audience', [])
        if audience:
            story.append(Paragraph("Target Audience:", self.styles['CustomHeading2']))
            for item in audience:
                story.append(Paragraph(f"• {self._sanitize_text(item)}", self.styles['CustomBullet']))
            story.append(Spacer(1, 0.1*inch))
        
        # PRD features
        features = self._prd_analysis.get('features', [])
        if features:
            story.append(Paragraph("Core Functionalities (from PRD):", self.styles['CustomHeading2']))
            for feature in features:
                story.append(Paragraph(f"• {self._sanitize_text(feature)}", self.styles['CustomBullet']))
            story.append(Spacer(1, 0.1*inch))
        
        # Project Scope
        scope = self._prd_analysis.get('scope', [])
        if scope:
            story.append(Paragraph("Project Scope:", self.styles['CustomHeading2']))
            for item in scope:
                story.append(Paragraph(f"• {self._sanitize_text(item)}", self.styles['CustomBullet']))
        
        # If nothing was found, show a placeholder
        if not (goals or audience or features or scope):
            story.append(Paragraph("No structured business alignment data found in PRD. Consider explicitly defining Goals, Audience, and Features.", self.styles['CustomBody']))
    ...
    def _create_sequence_diagram_section(self) -> List:
        ...
        try:
            ...
            logger.info(f"Generating sequence diagram for entity: {selected_entity} with {len(endpoints)} endpoints")
            diagram_generator = ArchitectureDiagramGenerator(self.output_dir)
            ...
            logger.info(f"Sequence diagram generated at: {diagram_path}")
            ...
            from reportlab.platypus import Image
            if os.path.exists(diagram_path):
                logger.info(f"Embedding sequence diagram image: {diagram_path}")
                img = Image(diagram_path, width=7*inch, height=5*inch)
                story.append(img)
                story.append(Spacer(1, 0.2*inch))
            else:
                logger.error(f"Sequence diagram file not found: {diagram_path}")
                story.append(Paragraph(f"Sequence diagram could not be loaded from: {diagram_path}", self.styles['CustomBody']))
            ...
            story.append(Paragraph("Sequence Diagram Generation Error", self.styles['CustomHeading2']))
            story.append(Paragraph(f"Could not generate sequence diagram: {str(e)}", self.styles['CustomBody']))
            story.append(Paragraph("The system attempted to create a visual sequence diagram but encountered an error. This may be due to missing dependencies or configuration issues.", self.styles['CustomBody']))
