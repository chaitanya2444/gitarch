...
class ArchitectureDiagramGenerator:
        ...
        try:
            ...
            for step in steps:
                try:
                    ...
                    logger.error(f"Error drawing step {step_number}: {str(step_error)}")
                    continue
            ...
            # Create a fallback diagram path
            filename = f"sequence_diagram_fallback_{int(time.time())}.png"
            filepath = os.path.join(self.output_dir, filename)
            
            # Create a simple fallback diagram
            try:
                fig, ax = plt.subplots(1, 1, figsize=(10, 6))
                ax.text(0.5, 0.5, f'Sequence Diagram\n{project_name}\nGeneration Error: {str(e)}', 
    ...
    def _detect_available_operations(self, endpoints: List[Dict], entity: str) -> List[str]:
        ...
        try:
            ...
            logger.error(f"Error detecting operations: {str(e)}")
            return ['CRUD']
    ...
    def _generate_dynamic_steps(self, endpoints: List[Dict], entity: str) -> List[Dict]:
        ...
        try:
            ...
            logger.error(f"Error generating dynamic steps: {str(e)}")
            return self._generate_generic_flow(entity)
    ...
    def _generate_generic_flow(self, entity: str) -> List[Dict]:
        ...
        try:
            ...
            logger.error(f"Error generating generic flow: {str(e)}")
            # Return minimal flow as last resort
            return [
                {'src': 0, 'dst': 1, 'label': 'User request', 'type': 'action'},
                {'src': 1, 'dst': 2, 'label': 'API call', 'type': 'req'},
                {'src': 2, 'dst': 1, 'label': 'Response', 'type': 'res'},
                {'src': 1, 'dst': 0, 'label': 'Display result', 'type': 'res'},
            ]
